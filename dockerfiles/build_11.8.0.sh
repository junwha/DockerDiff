...
# docker build -t junwha/dslice-base --build-arg DOCKER_CUDA_VERSION=$CUDA_VERSION -f Dockerfile.base ./
# ../dslice push junwha/dslice-base

for PY_VERSION in 3.9; do # 3.8 3.10 3.12; do
...
for PY_VERSION in 3.9; do # 3.8 3.10 3.12; do
    for TORCH_VERSION_PAIR in "${TORCH_VERSION_PAIRS[@]}"; do
        ...
        docker build -t junwha/dslice-base-torch:py$PY_VERSION-torch$TORCH_VERSION \
            ...
            -f Dockerfile.torch ./
        ../dslice save junwha/dslice-base-torch:py$PY_VERSION-torch$TORCH_VERSION
    done
done
